...
__global__ void mvt_kernel1(DATA_TYPE *a, DATA_TYPE *x1, DATA_TYPE *y_1,
                            Policy con, int app_id, int flag)
{
             ...
             DATA_TYPE *x2_outputFromGpu, const int &app_id)
{
  ...
  if (flag)
  {
    ...
    //mvt_kernel1<<<task1.block_num, block>>>(a_gpu, x1_gpu, y_1_gpu, task1, app_id, -1);
  }
  else
  {
	...
	cudaEvent_t sk_end;
	cudaEventCreate(&sk_end);
    
      	mvt_kernel1<<<task1_1.block_num, block, 0, stream1>>>(a_gpu, x1_gpu,y_1_gpu, task1_1);
      cudaEventRecord(sk_end, stream1); // Record the event after the kernel has finished executing

	// Stall stream2 until stream1 has finished executing
    cudaStreamWaitEvent(stream2, sk_end, 0);	
      cudaEventDestroy(sk_end); 
    
      mvt_kernel1<<<task1_1.block_num, block, 0, stream2>>>(a_gpu, x1_gpu,y_1_gpu, task1_2);
